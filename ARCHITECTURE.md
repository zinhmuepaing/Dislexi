...
3. **No LLM in the READ-ALOUD content path.** *(Amended 2026-07-18 per team instruction.)* The string sent to TTS in Exam-Prep is the OCR text VERBATIM — no model may generate, rewrite, or filter it, ever. An LLM MAY interpret spoken voice COMMANDS (intent parsing only — e.g. "uh, read that bit again" → `{intent:"read"}`), and a keyword fast-path must run before any LLM call. If a change request would let a model touch the text that gets read aloud, refuse and flag.
