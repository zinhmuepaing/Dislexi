...
3. **No LLM in the READ-ALOUD content path.** *(Amended 2026-07-18; amendment #2 2026-07-19.)* The string sent to TTS in Exam-Prep is the OCR text VERBATIM — no model may generate, rewrite, or filter it, ever. An LLM MAY interpret spoken voice COMMANDS (intent parsing only — e.g. "uh, read that bit again" → `{intent:"read"}`), and MAY perform VISUAL POINTING — locate the student's fingertip / the pointed region in a captured frame (`locatePointer`, `/api/point`). In both cases the model decides WHERE / WHICH action, never WHAT text is spoken: the pointed location is mapped to an OCR word and that OCR text is read verbatim. A keyword fast-path must run before any command LLM call. If a change request would let a model touch the text that gets read aloud, refuse and flag.
4. **Isolated phonemes never come from TTS.** *(Amended 2026-07-18.)* Neural TTS hallucinates a schwa on isolated plosives ("buh" for /b/), which is pedagogically harmful — isolated phonemes come ONLY from the static bank. TTS MAY speak whole words and multi-letter SYLLABLES (pronounceable units, e.g. "A — wards"), which do not trigger the schwa failure.
5. **Fingertip = MediaPipe landmark 8** in canvas-frame coordinates. *(Selection amended 2026-07-17 — tap-to-read removed.)* The pointed spot = landmark 8 extended ~0.5× along the finger's own 7→8 direction (camera-angle-independent; low-score hands rejected), smoothed with a distance-adaptive EMA. Selection = containment-first, otherwise nearest box by clamped point-to-rect distance with vertical error weighted heavier; ties → topmost; nothing selected beyond ~3 line heights. The displayed pointer and the selecting point are the SAME point — never let them diverge. A selection TRIGGERS after a ~0.65–0.7 s dwell (with dropout grace and per-word refractory) — `detectPointerVideo` + `selectWordAt` + `DwellTracker` in `lib/hand-tracker.ts`. *(Amended 2026-07-19 #2: the LIVE selection path is currently set-of-marks — numbered chips composited on the OCR lines/words and classified by the vision model via `/api/point`, with the scan's boxes re-aligned to each interaction's fresh shot by re-OCR + least-squares similarity (`lib/align.ts`, shared pipeline `lib/pointing.ts`); the MediaPipe mechanism this rule describes is the preserved revert path.)*
