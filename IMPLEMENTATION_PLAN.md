...
- [x] **S0 — Plan + rule-3 amendment #2 in-repo** (this section; ARCHITECTURE
  §7 rule 3 + CLAUDE.md). Gate: docs only.
- [x] **S1 — CRITICAL: vision-LLM pointing.** New `locatePointer(imageBase64)`
...
- [x] **S2 — Highlight accuracy (missing first char + trailing blank).** Root
...
- [x] **S3 — Overlay colour + shape conflict.** New `--point` token
...
- [x] **S4 — Stray blue horizontal line (right of camera).** Reproduce with a
...
- [x] **S5 — Bigger camera view.** Raise the canvas cap (~42dvh → ~54dvh),
...
- [x] **S6 — Landing + global modernization.** Remove "Tech4City 2026"; cut
...
- [x] **S7 — Tutoring on-paper working + text toggle (DeskTutor-style).**
...
- [x] **S8 — Autopsy quiz "point at it" tracking + stuck-highlight bug.**
